...
> 这里先保留图片位置。发布 GitHub 前，可以把示例图片放到 `docs/images/` 目录，并替换下面的路径。

### 检测流程截图

![GUI 检测界面示例](docs/images/gui_demo.png)

### 异常热力图

![异常热力图示例](docs/images/heatmap_example.png)

### 原图叠加图

![热力图叠加图示例](docs/images/overlay_example.png)
...
下面结果来自本地实验输出 `barcode_vae_app/runs/refine_search/nc50_50_w11`，可作为 README 展示用的示例指标。后续如果重新训练，可以直接替换为最新结果。

| 指标 | 数值 |
|---|---:|
| 测试 OK 数量 | 150 |
| 测试 NG 数量 | 280 |
| 阈值 threshold | 0.001276 |
| OK 误判为 NG | 4 |
| NG 漏检为 OK | 20 |
| OK 通过率 | 97.33% |
| NG 检出率 | 92.86% |
...
标定集上的平衡误差约为 `4.88%`。该结果仅代表当前数据拆分和当前模型配置，不代表所有实际生产场景的泛化效果。
